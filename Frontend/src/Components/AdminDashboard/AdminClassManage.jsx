...
const AdminClassManage = () => {
  ...
  const handleAddSubjectToList = () => {
    if (!courseTitle.trim() || !courseCode.trim() || !courseCredit || !courseType) {
  ...
  const handleAddSubjectToExisting = async (classNumber, subjectObj) => {
    if (!subjectObj.title || !subjectObj.code || !subjectObj.credit || !subjectObj.type) {
  ...
  const handleDeleteClass = async (classNum) => {
    if (!window.confirm(`Are you sure you want to delete Class ${classNum} and all its courses?`)) return;
    try {
      setLoading(true);
  ...
  const handleRemoveSubject = async (classNum, subject) => {
    if (!window.confirm(`Remove ${subject.title} from Class ${classNum}?`)) return;
    try {
      setLoading(true);
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      <div className="bg-white rounded-xl shadow-md overflow-hidden">
        <div className="p-6">
          <h1 className="text-2xl font-bold text-gray-800 mb-6">Semester & Course Management</h1>
          ...
          {activeTab === "create" && (
            <div className="mb-8 p-6 bg-gray-50 rounded-lg border border-gray-200">
              <h2 className="text-xl font-semibold text-gray-800 mb-4">Create New Semester</h2>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                <div>
                  ...
                  />
                  <p className="mt-1 text-xs text-gray-500">Format: Degree_Semester (e.g., Bsc_1.1)</p>
                </div>
                
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Course Details *</label>
                    <input
                      type="text"
              ...
              {subjectList.length > 0 && (
                <div className="mb-6">
                  <h3 className="text-sm font-medium text-gray-700 mb-3">Selected Courses:</h3>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    {subjectList.map((sub, idx) => (
                      <div key={idx} className="flex justify-between items-center bg-blue-50 border border-blue-100 px-4 py-2 rounded-lg">
                        <div>
                          <p className="font-medium text-blue-800">{sub.title}</p>
                          <p className="text-xs text-blue-600">{sub.code} • {sub.credit} credit • {sub.type}</p>
                        </div>
                        <button
          ...
          {activeTab === "manage" && (
            <div>
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-semibold text-gray-800">Existing Semesters</h2>
                <button 
                  onClick={() => setActiveTab("create")}
              ...
              {loading && !classes.length ? (
                ...
                <div className="text-center py-12 bg-gray-50 rounded-lg border border-gray-200">
                  ...
                  <h3 className="mt-2 text-lg font-medium text-gray-900">No semesters found</h3>
                ...
                <div className="space-y-6">
                  {classes.map((cls) => (
                    <div key={cls._id} className="border rounded-lg overflow-hidden">
                      ...
                      <div className="p-6">
                        {cls.subjects.length > 0 ? (
                          <div className="mb-6">
                            <h4 className="text-sm font-medium text-gray-700 mb-3">Courses in this semester:</h4>
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                  {cls.subjects.map((sub, idx) => (
                                    <tr key={idx}>
                                      ...
                                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{sub.credit}</td>
                                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        <span className={`px-2 py-1 text-xs rounded-full ${sub.type === 'Lab' ? 'bg-purple-100 text-purple-800' : 'bg-green-100 text-green-800'}`}>
                                          {sub.type}
                                        </span>
                                      </td>
                                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        ...
                        <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                          <h4 className="text-sm font-medium text-gray-700 mb-3">Add New Course to Semester {cls.class}</h4>
                          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                            <input
                              ...
                              onChange={(e) =>
                                setNewSubjectInputs((prev) => ({
                                  ...prev,
                                  [cls.class]: { ...prev[cls.class], title: e.target.value }
                                }))
                              }
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setNewSubjectInputs((prev) => ({
                                  ...prev,
                                  [cls.class]: { ...prev[cls.class], code: e.target.value }
                                }))
                              }
                              className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition"
                            />
                            <input
                              ...
                              onChange={(e) =>
                                setNewSubjectInputs((prev) => ({
                                  ...prev,
                                  [cls.class]: { ...prev[cls.class], credit: e.target.value }
                                }))
                              }
                              className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition"
                            />
                            <select
                              ...
                              onChange={(e) =>
                                setNewSubjectInputs((prev) => ({
                                  ...prev,
                                  [cls.class]: { ...prev[cls.class], type: e.target.value }
                                }))
                              }
                              className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition"
                            >
                          ...
                          <button
                            onClick={() => handleAddSubjectToExisting(cls.class, newSubjectInputs[cls.class])}
                            disabled={!newSubjectInputs[cls.class]?.title || !newSubjectInputs[cls.class]?.code || !newSubjectInputs[cls.class]?.credit || !newSubjectInputs[cls.class]?.type}
